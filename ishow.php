...
//		  if (empty($text) {} else …
            ...
            list($pattern, $duration, $swipe, $dtype, $dlink) = $this->yellow->toolbox->getTextArgs($text);
			if (empty($pattern)) {
			...
			if (count($files)) {
				if (empty($duration)) $duration = 4;
				if (empty($swipe)) $swipe = 1;
				if (empty($dtype)) $dtype = 1;
				if (empty($dlink)) $dlink = "";
			    
			    $fcount = count($files);
				//	Basic style preparation
				$output = "<style>.iout {width: 100%; margin:0 auto; position:relative; overflow:hidden;}.ishow div{width:100%;position:absolute; height:auto;text-align:center;transform:translateX(100%);}.fill {border: solid rgb(230,230,230) 1px; border-radius:6px;}.fill img {visibility:hidden;}.ishow div img, .fill img {max-width:100%;height: auto;padding:3px;margin:0 auto;display:block;overflow: hidden;}";
				...
				switch ($dtype) {				
					case "left":
						$output .= "0%{transform: translateX(100%); }";
						$output .= $sep."% {transform: translateX(0);}";
						$output .= $seq."% {transform: translateX(0);}";
						$output .= $seq+$sep."%{transform: translateX(-100%); }";
						$output .= "100%{transform: translateX(-100%); }";
						break;
					case "right":
						$output .= "0%{transform: translateX(-100%); }";
						$output .= $sep."% {transform: translateX(0);}";
						...
						break;
					case "up":
						$output .= "0%{transform: translate(0%,100%);}";
						$output .= $sep."% {transform: translateY(0%)}";
						$output .= $seq."% {transform: translate(0%,0%);}";
						$output .= $seq+$sep."%{transform: translateY(-100%)}";
						$output .= "100%{transform: translateY(-100%)}";
						break;
					case "down":
						$output .= "0%{transform: translate(0%,-100%);}";
						$output .= $sep."%{transform: translateY(0%)}";
					...
					case "zoom":
						...
						$output .= "100%{transform: translateX(-100%) scale(1);opacity: 0;}";
						break;	
					case "fly":
						$output .= "0%{transform: scale(0.5) translate(-150%, -90%) skew(45deg, 0deg) rotate(180deg);}";
						$output .= $sep."% {transform: translateX(0%) rotate(0deg);}";
						$output .= $seq."% {transform: translateX(0%);}";
						$output .= $seq+$sep."%{transform: scale(0.5) translate(150%, -90%) skew(-45deg, 0deg) rotate(-180deg);}";
						$output .= "100%{transform: translateX(100%,0%);}";
						break;		
					case "show":
					default:
				...
				foreach ($files as $file) {	
					...
					if (0 == $i) { 	$first = $file->getLocation(true); }
					// image container
					$output .= "<div " . ($dlink ? "onclick=\"location.href='". $dlink.pathinfo($file->getLocation(true),PATHINFO_FILENAME) . "';\"  " : "")  . "id=\"ic".$i."\"><img src=\"". $file->getLocation(true) .	"\"alt=\"\"/></div>\n";
					$i++;
				}
